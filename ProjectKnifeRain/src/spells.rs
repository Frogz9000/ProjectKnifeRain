...
use bevy_rapier3d::prelude::{
    ActiveEvents, Collider, CollisionEvent, Damping, ExternalImpulse, GravityScale, RigidBody,
    Sleeping, Velocity,
};
...
const KNOCKBACK_STRENGTH: f32 = 10.0;
...
fn fireball_collision_process(
    ...
    for event in collision_event.read() {
        if let CollisionEvent::Started(fireball_entity, hit_entity, _) = event {
            ...
            let Ok(fireball_transform) = fireball_query.get(fireball_entity) else {
                continue;
            };
            ...
            for (mut impulse, hit_transform, velocity) in knockback_entities.iter_mut() {
                ...
                if distance <= KNOCKBACK_RADIUS {
                    ...
                    impulse.impulse =
                        knockback_dir.normalize_or_zero() * KNOCKBACK_STRENGTH * falloff;
                }
            }
